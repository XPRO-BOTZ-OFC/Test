...
/**
 * Check if message contains button content
 */
...
/**
 * Get biz node for button messages
 */
...
export const generateWAMessageContent = async (message, options) => {
    ...
    // Handle BUTTONS - Regular buttons with media support
    if (hasNonNullishProperty(message, 'buttons') && !!message.buttons) {
        ...
        // Check if we have media from prepareWAMessageMedia
        const hasMedia = Object.keys(m).length > 0 && 
                       (m.imageMessage || m.videoMessage || m.documentMessage || m.audioMessage);
        
        if (hasMedia) {
            // This is a media message with buttons
            if (m.imageMessage) {
                buttonsMessage.imageMessage = m.imageMessage;
                buttonsMessage.headerType = ButtonType.IMAGE;
            } else if (m.videoMessage) {
                buttonsMessage.videoMessage = m.videoMessage;
                buttonsMessage.headerType = ButtonType.VIDEO;
            } else if (m.documentMessage) {
                buttonsMessage.documentMessage = m.documentMessage;
                buttonsMessage.headerType = ButtonType.DOCUMENT;
            }
            
            // Use caption as content text
        ...
        if (hasNonNullishProperty(message, 'title') && !!message.title) {
            buttonsMessage.text = message.title;
            buttonsMessage.headerType = ButtonType.TEXT;
        }
        
        if (hasNonNullishProperty(message, 'footer') && !!message.footer) {
            buttonsMessage.footerText = message.footer;
        }
        
        if (hasNonNullishProperty(message, 'contextInfo') && !!message.contextInfo) {
            buttonsMessage.contextInfo = message.contextInfo;
        }
        
        if (hasNonNullishProperty(message, 'mentions') && !!message.mentions) {
    ...
    // Handle INTERACTIVE BUTTONS
        ...
        if (hasNonNullishProperty(message, 'text')) {
            ...
            if (hasNonNullishProperty(message, 'title') || hasNonNullishProperty(message, 'subtitle')) {
                interactiveMessage.header = {
                    ...
                    hasMediaAttachment: !!(message.media || (Object.keys(m).length > 0 && (m.imageMessage || m.videoMessage))),
                };
        ...
        if (hasNonNullishProperty(message, 'mentions') && !!message.mentions) {
            interactiveMessage.contextInfo = interactiveMessage.contextInfo || {};
...
export const generateWAMessageFromContent = (jid, message, options) => {
    ...
    return proto.WebMessageInfo.fromObject(messageJSON);
};
...
export const downloadMediaMessage = async (message, type, options, ctx) => {
    ...
    async function downloadMsg() {
        ...
        if (type === 'buffer') {
            // Pre-allocate growing buffer — avoids chunk array + Buffer.concat double-spike
            const PAGE = 65536;
