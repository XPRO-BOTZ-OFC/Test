...
export const extractImageThumb = async (bufferOrFilePath, width = 32) => {
    ...
    // If input is a Readable stream, handle it efficiently
    if (bufferOrFilePath instanceof Readable) {
        if ('sharp' in lib && typeof lib.sharp?.default === 'function') {
            // Sharp can process streams directly without buffering
            const pipeline = lib.sharp.default();
            bufferOrFilePath.pipe(pipeline);
            const dimensions = await pipeline.metadata();
            const buffer = await pipeline.resize(width).jpeg({ quality: 50 }).toBuffer();
            return {
                buffer,
                original: {
            ...
            // Fallback: buffer the stream (memory heavy but unavoidable)
...
/**
 * Generates waveform for audio. For long audio (>120 seconds) it skips processing
 * to avoid out-of-memory errors, logging a warning.
 */
export async function getAudioWaveform(buffer, logger) {
    try {
        // Check duration first – if too long, skip to save memory
        let duration = 0;
        try {
            duration = await getAudioDuration(buffer);
        } catch (e) {
            logger?.debug('Failed to get audio duration for waveform, continuing anyway');
        }
        const MAX_DURATION_SEC = 120; // 2 minutes – typical voice note length
        if (duration > MAX_DURATION_SEC) {
            logger?.warn(`Audio duration ${duration}s exceeds ${MAX_DURATION_SEC}s, skipping waveform generation to prevent OOM`);
            return new Uint8Array(0); // empty waveform – feature preserved for short audios
        }
