...
// ─── Module-level caches ────────────────────────────────────────────────────
// Cache the image processing library after first load — avoids re-importing
// jimp + sharp on every thumbnail/profile-pic call (each import() allocates).
let _imageLibCache = null;
...
// Cache audio library imports — avoids re-importing on every audio message
let _musicMetadata = null;
let _audioDecoder = null;
const getMusicMetadata = async () => {
    if (!_musicMetadata) _musicMetadata = await import('music-metadata');
...
export const extractImageThumb = async (bufferOrFilePath, width = 32) => {
    ...
    if ('sharp' in lib && typeof lib.sharp?.default === 'function') {
        // sharp accepts Readable streams AND file paths natively — skip toBuffer entirely
    ...
    // jimp does not support streams — only buffer if we must fall back to it
...
/**
 * Get audio duration — prefers file path to avoid loading into RAM.
 * Falls back to Buffer or Stream when needed.
 */
export async function getAudioDuration(buffer) {
    const musicMetadata = await getMusicMetadata();
    const options = { duration: true };
    // Prefer file path — reads directly from disk, no RAM spike
    if (typeof buffer === 'string') {
 ...
 * Uses cached audio-decode import to avoid re-importing on every PTT message.
 * When given a file path, reads via stream — never loads full file into RAM.
 */
export async function getAudioWaveform(buffer, logger) {
    try {
        const decoder = await getAudioDecoder();
        let audioData;
        if (Buffer.isBuffer(buffer)) {
            audioData = buffer;
        } else if (typeof buffer === 'string') {
            // Stream file into buffer — avoids fs.readFileSync spike
        ...
        // Release raw audio data immediately — decoder has its own copy now
        audioData = null;
        ...
        // Use reduce instead of Math.max(...spread) — spread can overflow stack on long audio
        const multiplier = Math.pow(filteredData.reduce((a, b) => Math.max(a, b), 0), -1);
 ...
 * Uses a pre-allocated growing buffer instead of chunks[] + Buffer.concat —
 * halves peak RAM by avoiding the double-allocation spike.
 */
export const toBuffer = async (stream) => {
    const PAGE = 65536; // 64KB initial page
    let buf = Buffer.allocUnsafe(PAGE);
    let pos = 0;
    for await (const chunk of stream) {
        const needed = pos + chunk.length;
        if (needed > buf.length) {
            const next = Buffer.allocUnsafe(Math.max(buf.length * 2, needed));
 ...
 * Returns a Transform stream — caller can pipe directly to disk or another
 * stream without ever loading the full file into RAM.
 */
export const downloadEncryptedContent = async (downloadUrl, { cipherKey, iv }, { startByte, endByte, options } = {}) => {
    ...
    const headers = {
        ...(headersInit
            ? Array.isArray(headersInit) ? Object.fromEntries(headersInit) : headersInit
            : {}),
    ...
    // null sentinel avoids Buffer.from([]) allocation on every chunk
    ...
    const output = new Transform({
        transform(chunk, _, callback) {
            let data = remainingBytes ? Buffer.concat([remainingBytes, chunk]) : chunk;
            const decryptLength = toSmallestChunkSize(data.length);
            remainingBytes = decryptLength < data.length ? data.subarray(decryptLength) : null;
