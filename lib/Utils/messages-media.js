...
// --- NEWSLETTER FIX: Restored original getRawMediaUploadData ---
...
export const extractImageThumb = async (bufferOrFilePath, width = 32) => {
    ...
    // If input is a Readable stream, handle it efficiently with Sharp (no buffering)
...
// Optimised audio duration: uses a limited read for streams (first 2 MB)
export async function getAudioDuration(buffer) {
    ...
    try {
        if (Buffer.isBuffer(buffer)) {
            ...
            // For streams, only read first 2 MB – enough for duration metadata
...
// Waveform: skip if audio longer than 2 minutes, otherwise decode only needed part
export async function getAudioWaveform(buffer, logger) {
    try {
        const duration = await getAudioDuration(buffer);
        const MAX_DURATION_SEC = 120;
        if (duration > MAX_DURATION_SEC) {
            logger?.warn(`Audio duration ${duration}s > ${MAX_DURATION_SEC}s, skipping waveform generation to prevent OOM`);
